...
async function AdminDashboardData({ assignId }: { assignId?: string }) {
  ...
  const selectedEmployee = assignId ? adminData.find((e) => e.id === assignId) : null;
          ...
          <Card key={emp.id}>
            ...
            <CardContent>
              {emp.salaries && emp.salaries.length > 0 ? (
                <div>
                  <p className="text-2xl font-bold">{formatINR(emp.salaries[0].baseAmount)}</p>
              ...
              <Button
                ...
                nativeButton={false}
                render={<Link href={`?assignId=${emp.id}`} scroll={false}>Update Salary</Link>}
              />
            </CardContent>
          </Card>
        ))}
...
async function EmployeeDashboardData({ employeeId }: { employeeId: string }) {
  const salariesData = await db.query.salaries.findMany({
    where: eq(salariesSchema.employeeId, employeeId),
    orderBy: [desc(salariesSchema.effectiveDate), desc(salariesSchema.createdAt)],
  });
  
  const attendanceData = await db.query.attendance.findMany({
    where: eq(attendanceSchema.employeeId, employeeId),
  });
  ...
  const dailyRate = expectedWorkingDays > 0 ? monthlyBase / expectedWorkingDays : 0;
